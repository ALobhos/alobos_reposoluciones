...
void
revision (int jugadas, int registro[64], int jugador, int tablero[8][8]){
	system ("clear");
...
void
pantalla_final (int ganador, int registro[64], int jugadas, int inicial, int tablero[8][8]){
	...
	switch (seleccion){
		case 1:
			revision (jugadas, registro, inicial, tablero);
			break;
		case 2:
			system ("clear");
			break;
...
playmaker (int tablero[8][8], int jugador, int ia, int jugadas, int registro[64]){
	...
	// se determina si le toca a un jugador humano o a la IA
		if (jugador == 1){
			...
			printf ("¿En que columa suelta la ficha?");
			scanf ("%d", &jugada);
			}
...
nucleo (int ia){
	...
	mostrar_tablero(tablero);
	pantalla_final(termino, registro, jugadas, inicial, tablero);
}
...
creditos(){
	...
	printf ("\n");
	sleep (1);
	printf ("\t\t\t\t\t\t\t\tAutor: Arturo Lobos\n");
	printf ("\n");
	sleep (1);
	printf ("\t\t\t\t\t\t\t\tProfesor: Fabio Durán\n");
	printf ("\n");
	sleep (1);
	printf ("\t\t\t\t\t\t\tProyecto final Soluciones Algorítmicas\n");
	printf ("\n");
	sleep (1);
	printf ("\t\t\t\t\t\t\tEscuela de Ingeniería Civil en Bioinformática\n");
	printf ("\n");
	getchar();
	getchar();
	system ("clear");
